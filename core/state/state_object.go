...
type stateObject struct {
	...
	localDelta     []rawdb.FlatDeltaEntry
	localDeltaKeys map[string]struct{}
...
func (s *stateObject) recordLocalDelta(key []byte, old []byte, existed bool) {
	if !s.db.Database().ObservationMode() {
		return
	}
	if s.localDeltaKeys == nil {
		s.localDeltaKeys = make(map[string]struct{})
	}
	k := string(key)
	if _, ok := s.localDeltaKeys[k]; ok {
		return
	}
	s.localDeltaKeys[k] = struct{}{}
	entry := rawdb.FlatDeltaEntry{
		Key:        append([]byte(nil), key...),
		OldValue:   append([]byte(nil), old...),
		OldExisted: existed,
	}
	s.localDelta = append(s.localDelta, entry)
}
