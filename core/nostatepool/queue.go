...
func (q *queue) promoteExecutables(accounts []common.Address, gasLimit uint64, nonces *noncer) ([]*types.Transaction, []common.Hash, []common.Address) {
	...
	for _, addr := range accounts {
		...
		// Drop all transactions that are deemed too old (below tracked pool nonce)
		forwards := list.Forward(nonces.get(addr))
		for _, tx := range forwards {
			dropped = append(dropped, tx.Hash())
		}
		log.Trace("Removing old queued transactions", "count", len(forwards))

		// Drop transactions that exceed gas limit. Balance checks are disabled in no-state mode.
		drops, _ := list.Filter(uint256.NewInt(0).SetAllOne(), gasLimit)
