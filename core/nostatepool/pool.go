...
// Package nostatepool implements a no-state variant of legacypool.
package nostatepool
...
func (pool *LegacyPool) reserve(addr common.Address) error {
	if pool.reserver == nil {
		return nil
	}
	return pool.reserver.Hold(addr)
}

func (pool *LegacyPool) release(addr common.Address) {
	if pool.reserver == nil {
		return
	}
...
func (pool *LegacyPool) reservedByOther(addr common.Address) bool {
	if pool.reserver == nil {
		return false
	}
	return pool.reserver.Has(addr)
}
...
func (pool *LegacyPool) validateTx(tx *types.Transaction) error {
	...
	if next := pool.pendingNonces.get(from); next > tx.Nonce() {
...
func (pool *LegacyPool) checkDelegationLimit(tx *types.Transaction) error {
	...
	// In no-state mode we can only rely on pending in-pool authorization tracking.
	if !pool.all.hasAuth(from) {
		return nil
	}
...
func (pool *LegacyPool) add(tx *types.Transaction) (replaced bool, err error) {
	...
	if !hasPending && !hasQueued {
		if err := pool.reserve(from); err != nil {
			return false, err
		}
		defer func() {
	...
	if list := pool.pending[from]; list != nil && list.Contains(tx.Nonce()) {
		...
		pool.pendingNonces.setBaseIfLower(from, tx.Nonce())
		return old != nil, nil
	}
...
func (pool *LegacyPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) bool {
	...
	pool.pendingNonces.setBaseIfLower(addr, tx.Nonce())
...
func (pool *LegacyPool) removeTx(hash common.Hash, outofbound bool, unreserve bool) int {
	return pool.removeTxWithNonceMode(hash, outofbound, unreserve, true)
}

// removeTxWithNonceMode removes a single transaction from the queue, moving
// subsequent invalid transactions back to the future queue. The touchNonce flag
// controls whether pendingNonces should be adjusted during removal.
func (pool *LegacyPool) removeTxWithNonceMode(hash common.Hash, outofbound bool, unreserve bool, touchNonce bool) int {
...
func (pool *LegacyPool) reset(oldHead, newHead *types.Header) {
	...
	// Canonical extension (non-reorg): collect newly included transactions too.
	if oldHead != nil && newHead != nil && oldHead.Hash() == newHead.ParentHash {
		if txs, ok := pool.collectIncludedOnCanonicalExtension(oldHead, newHead); ok {
	...
	pool.pendingNonces = newNoncer()
	pool.evictIncluded(included)
	pool.rebuildNonceBaseline()
...
// collectIncludedOnCanonicalExtension returns transactions included between
// oldHead (exclusive) and newHead (inclusive) when newHead extends oldHead.
func (pool *LegacyPool) collectIncludedOnCanonicalExtension(oldHead, newHead *types.Header) (types.Transactions, bool) {
	add := pool.chain.GetBlock(newHead.Hash(), newHead.Number.Uint64())
	if add == nil {
		log.Warn("Transaction pool reset with missing canonical new head", "number", newHead.Number, "hash", newHead.Hash())
		return nil, false
	}
	var included types.Transactions
	for add.NumberU64() > oldHead.Number.Uint64() {
		included = append(included, add.Transactions()...)
		add = pool.chain.GetBlock(add.ParentHash(), add.NumberU64()-1)
		if add == nil {
			log.Warn("Transaction pool reset with missing canonical ancestor", "old", oldHead.Hash(), "new", newHead.Hash())
			return nil, false
		}
	}
	if add.Hash() != oldHead.Hash() {
		return nil, false
	}
	return included, true
}

// evictIncluded removes all transactions that are already included in newly
// accepted blocks from both pending and queue.
func (pool *LegacyPool) evictIncluded(included types.Transactions) {
	if len(included) == 0 {
		return
	}
	hits := make(map[common.Hash]struct{}, len(included))
	for _, tx := range included {
		hits[tx.Hash()] = struct{}{}
	}
	var removals []common.Hash
	pool.all.Range(func(hash common.Hash, tx *types.Transaction) bool {
		if _, ok := hits[hash]; ok {
			removals = append(removals, hash)
		}
		return true
	})
	for _, hash := range removals {
		pool.removeTxWithNonceMode(hash, true, true, false)
	}
}

// rebuildNonceBaseline reconstructs no-state nonce baselines from the current
// pool contents. For accounts with pending transactions, pending starts are
// preferred; queue baselines are only used when pending is absent.
func (pool *LegacyPool) rebuildNonceBaseline() {
	base := make(map[common.Address]uint64, len(pool.pending)+len(pool.queue.queued))
	for addr, list := range pool.pending {
		txs := list.Flatten()
		if len(txs) == 0 {
			continue
		}
		base[addr] = txs[0].Nonce()
	}
	...
	pool.pendingNonces.setBaseAll(base)
}
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	promotable, dropped, removedAddresses := pool.queue.promoteExecutables(accounts, gasLimit, pool.pendingNonces)
...
func (pool *LegacyPool) demoteUnexecutables() {
	...
	gasLimit := pool.currentHead.Load().GasLimit
	maxBalance := uint256.NewInt(0).SetAllOne()
	for addr, list := range pool.pending {
		...
		// Drop all transactions that exceed gas limit, and queue any invalids back for later.
		// Balance-based filtering is disabled in no-state mode.
		drops, invalids := list.Filter(maxBalance, gasLimit)
